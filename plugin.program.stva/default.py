		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count18 > 0:
					...
					addFile('[B]%s Base y Parches STVA para Kodi 18 Leia (%s)[/B]' % (state, count18), 'togglesetting',  'show18', themeit=THEME3)
					if SHOW18 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count15 > 0:
					...
					addFile('[B]%s Base y Parches LITE para Kodi 18 Leia (%s)[/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME3)
					if SHOW15 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        addFile('[COLOR lime][B]INSTALAR BASE[/B][/COLOR]'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def maintMenu(view=None):
	...
	addDir ('[I]<< Volver al Menu Principal STVA[/I]', icon=ICONMAINT, themeit=THEME2)	    
